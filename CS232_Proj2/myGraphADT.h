...
const double MAX_DIST = DBL_MAX;  // the max size of current distance in a vertex
...
protected:
    ...
    /* Recursive utility that does the actual work for DFTraversal */
...
public:
    ...
    /* Removes all vertices in main graph vector */
    void clear();
    ...
    /* Performs DFT with information printed */
...
struct edgeRep
{
    ...
    bool operator != (const edge& e) const { return !operator==(e); }       // ⚠️ 把e放进去前一个==overload function，再取反就等于!=了，聪明！
...
struct vertex    // Array cell structure for graph
{
    ...
    explicit vertex(V n)            // vertex Constructor
    {
        ...
        currDist = 0;               // used by finding shortest path
